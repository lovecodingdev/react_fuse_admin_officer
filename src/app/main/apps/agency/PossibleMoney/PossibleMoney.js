	...
	useEffect(() => {	
		if(!_.isEmpty(widgets) && !_.isEmpty(main)) {
			...
			if(widgets.Agency_PossibleMoney_Current_Level_Table) {
				...
				policies.map(policy => {
					if(policy.value !== 'Total') {
						...
						let bonusSum = 0;
						if(tabValue===0) {
							policySum = main[production][period][user][policy.value]['Policies'];
							premiumSum = main[production][period][user][policy.value]['Premium'];
							...
							if(bonus === 'Include Initial Bonus in Calculation') {
								bonusSum += parseFloat(main[production][period][user][policy.value]['Bonuses']);
							}							
						} else {
							users.map(user1 => { 	
								if(user1.belongTo === UID) {
			...
			if(widgets.Agency_PossibleMoney_Next_Level_Table) {	
				let totalBonus = 0;
				let tempRows = []; 	
				const rows = widgets.Agency_PossibleMoney_Current_Level_Table.table.rows;			
				...
				policies.map((policy, col) => {
					if(policy.value !== 'Total') {					
						...
						if(tabValue===0) {
							...
							if(bonus === 'Include Initial Bonus in Calculation') {
								nextBonus += parseFloat(main[production][period][user][policy.value]['Bonuses']);
							}
						} else {
				...
				widgets = {
					...widgets, Agency_PossibleMoney_Next_Level_Table: {
						...widgets.Agency_PossibleMoney_Next_Level_Table, table: {
							...widgets.Agency_PossibleMoney_Next_Level_Table.table, rows: 
								tempRows
						}
					}
				}
				...
				policies.map((policy, col) => {
					if(policy.value !== 'Total') {					
						...
						if(tabValue===0) {
							...
							if(bonus === 'Include Initial Bonus in Calculation') {
								maxBonus += parseFloat(main[production][period][user][policy.value]['Bonuses']);
							}
						} else {
				...
				widgets = {
					...widgets, Agency_PossibleMoney_Max_Level_Table: {
						...widgets.Agency_PossibleMoney_Max_Level_Table, table: {
							...widgets.Agency_PossibleMoney_Max_Level_Table.table, rows: 
								tempRows
						}
					}
				}
			...
			if(widgets.Agency_PossibleMoney_BonusPlan_Table && bonusPlans.length>0) {
				let tempBonusPlans = {};
				let dbName = '';
				if(tabValue === 0) {
					dbName = 'indDb';
				...
				rows.map((row, rowNum) => {
					...
					row.cells.map((cell, cellNum) => {
						...
						if(cellNum > 0)  {	console.log(policies[cellNum-1].value, `level ${rowNum+1}` )					
				...
				widgets = {
					...widgets, Agency_PossibleMoney_BonusPlan_Table: {
						...widgets.Agency_PossibleMoney_BonusPlan_Table, table: {
							...widgets.Agency_PossibleMoney_BonusPlan_Table.table, rows: 
								tempRows
						}
					}
				}
			...
			if(widgets.Agency_PossibleMoney_Chart) {		
				let tempDatasets = [];
				Object.keys(widgets.Agency_PossibleMoney_Chart.mainChart.TW.datasets).map((key, n) => {
					let temp = widgets.Agency_PossibleMoney_Chart.mainChart.TW.datasets[key];
					let tempData = [];
					if(temp.label !== 'Total') {
						tempData.push(widgets.Agency_PossibleMoney_Current_Level_Table.table.rows[4].cells[n+1].value);
						tempData.push(widgets.Agency_PossibleMoney_Next_Level_Table.table.rows[3].cells[n+1].value);
						tempData.push(widgets.Agency_PossibleMoney_Max_Level_Table.table.rows[3].cells[n+1].value);					
